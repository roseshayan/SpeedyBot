...
def public_callback(call):
    ...
    if action=='feedback' and len(parts)>2 and parts[2]=='user': C.BOT.answer_callback_query(call.id); C.BOT.send_message(call.from_user.id,'⭐ <b>از ۱ تا ۵ امتیاز بدهید:</b>',parse_mode='HTML',reply_markup=_rating_markup()); return True
    if action=='rate':
        if C.setting('feedback_enabled','1')!='1': C.BOT.answer_callback_query(call.id,'غیرفعال است.',show_alert=True); return True
