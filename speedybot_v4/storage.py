...
def init_db():
    c=db(); c.execute("PRAGMA journal_mode=WAL"); now=int(time.time())
    ...
    if not r:
        cur=c.execute("INSERT INTO plan_categories(name,active,sort_order,created_at,updated_at) VALUES ('عمومی',1,10,?,?)",(now,now)); cid=int(cur.lastrowid)
    else: cid=int(r[0])
...
def backfill_categories():
    c=db(); r=c.execute("SELECT id FROM plan_categories WHERE active=1 ORDER BY sort_order,id LIMIT 1").fetchone()
    if r: c.execute("UPDATE plans SET category_id=?,updated_at=COALESCE(updated_at,?) WHERE category_id IS NULL",(int(r[0]),int(time.time()))); c.commit()
    c.close()
